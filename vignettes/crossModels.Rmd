...
```{r normalized,warning=FALSE,fig.height=9,fig.width=7}
crossModels(NS,crossLevel="gene_id",design=design,cutoffMax=3,dataType="normalized",species="Homo.sapiens",numberSelected=100,saveReport=FALSE)
...
```{r TMM_normalized,warning=FALSE,fig.height=9,fig.width=7}
...
# Coercion into SummarizedExperiment and KallistoExperiment
Arkas can cast a kallistoExperiment class into SummarizedExperiment and vice versa
```{r coercion}

SE <- as(NS, "SummarizedExperiment")

kexp <- as(SE, "KallistoExperiment")

identical(kexp,as(SE,"KallistoExperiment"))
```
