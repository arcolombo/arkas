...
getHits <- function(fit, p) rownames(limma::topTable(fit, p=p, coef=2, n=nrow(NS)))
...
d <- edgeR::DGEList(counts=geneCounts, group=NS$senescent)
d <- edgeR::calcNormFactors(d)
d <- edgeR::estimateGLMTrendedDisp(d, NS_design)
d <- edgeR::estimateGLMTagwiseDisp(d, NS_design)
d <- edgeR::estimateGLMRobustDisp(d, NS_design) # superb, but slow!
edgeR::plotBCV(d)
```
...
```{r}
fit <- edgeR::glmFit(d, NS_design)
edgeResults <- edgeR::glmLRT(fit, coef=2)

getEdgeR <- function(fit, p) rownames(edgeR::topTags(fit, p=p, n=nrow(NS)))
...
```{r}
topVoom <- limma::topTable(voomResults, n=nrow(NS), coef=2)
...
```{r}
topEdgeR <- edgeR::topTags(edgeResults, n=nrow(NS))[[1]]
